...
> Status: experimental project. The API surface is intentionally minimal (`add`, `poll`, `size`, `clear`, `drain`): no batch operations, and the `PriorityQueue<T>` interface does not extend `java.util.Collection`.
